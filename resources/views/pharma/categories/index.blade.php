...
@section('title', 'Manage Product Categories | Pharma Dashboard')

@section('content')
<div class="row">
    <div class="col-md-12 mb-4">
        <h2 class="fw-bold" style="color: var(--primary-green);">Manage Product Categories</h2>
        <p class="text-muted">Create categories and subcategories to organize your product catalog.</p>
    </div>

    <div class="col-md-6 border-end">
        <div class="card border-0 shadow-sm p-4 mb-4">
            <h4 class="fw-bold mb-4" style="color: var(--primary-green);">Add Product Category</h4>
            <form action="{{ route('pharma.categories.product.store') }}" method="POST">
                @csrf
                <div class="mb-3">
        ...
        <div class="card border-0 shadow-sm p-4">
            ...
            <form action="{{ route('pharma.categories.product_subcategory.store') }}" method="POST">
                @csrf
                <div class="mb-3">
        ...
        <div class="card border-0 shadow-sm mb-3">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    ...
                    <form action="{{ route('pharma.categories.product.destroy', $category->id) }}" method="POST" onsubmit="return confirm('Deleting category will delete all its subcategories. Are you sure?');">
                        @csrf
                        @method('DELETE')
                ...
                <div class="d-flex flex-wrap gap-2">
                    @forelse($category->subcategories as $sub)
                        <div class="badge bg-light text-dark border d-flex align-items-center gap-2 p-2">
                            {{ $sub->name }}
                            <form action="{{ route('pharma.categories.product_subcategory.destroy', $sub->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
